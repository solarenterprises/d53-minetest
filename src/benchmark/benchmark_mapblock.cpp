...
static void allocateSome(MBContainer &vec, u32 n)
{
	...
	for (u32 i = 0; i < n; i++) {
		auto *mb = new MapBlock({(s16)(i & 0xff), 0, (s16)(i >> 8)}, nullptr);
		vec.push_back(mb);
	}
}
...
{
	const float MAX_DISTANCE = 30000;
	const float MAX_DISTANCE_SQR = MAX_DISTANCE * MAX_DISTANCE;

	const v3s16 offset = v3s16(MAP_BLOCKSIZE / 2);
	...
	for (MapBlock *block : vec) {
		...
		v3s16 pos = block->getPosRelativeRef();
		if (foo)
			pos += offset;

		if (pos.getDistanceFromSQ(v3s16(0)) > MAX_DISTANCE_SQR)
			continue;
